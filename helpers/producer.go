...
type putProducer struct {
    ...
    client *ds3.Client
    wg *sync.WaitGroup
    writeObjectMap map[string]WriteObject
    processedChunksTracker ProcessedChunksTracker
    waitingToBeTransferred BlobDescriptionQueue
}

func newPutProducer(jobMasterObjectList *models.MasterObjectList, writeObjects *[]WriteObject, queue *chan TransferOperation, strategy *WriteTransferStrategy, client *ds3.Client, wg *sync.WaitGroup) *putProducer {
    return &putProducer{
...
func (pp *putProducer) transferOperationBuilder(info createObjectInfo) TransferOperation {
    return func() {
        defer info.onDone()
...
func toWriteObjectMap(writeObjects *[]WriteObject) map[string]WriteObject {
    objectMap := make(map[string]WriteObject)
